...
- **Sonnet minimum**: Haiku's quality gap causes retries that cost more than using Sonnet directly. If budget-constrained, use Sonnet for everything rather than mixing in Haiku
...
Use the Worker Prompt Template below for every worker prompt.
...
- `blueprint config get staging_branch` for project config
- `blueprint detect-stack` for framework/language detection
- `blueprint time` for timestamps (never guess dates)
- `blueprint commit format <type> "<msg>"` for emoji formatting
...
|------|-------|----------|-------|------|---------|
| 0002 CLI | Phase 1 (foundation) | Single Subagent (Opus) | 7 | ~4 min | ~15% |
| 0002 CLI | Phase 2+3+4+6 (parallel) | 4 Parallel Subagents | 20 | ~6 min | ~20% |
| 0002 CLI | Phase 5 (complex) | Single Subagent (Opus) | 9 | ~3 min | ~10% |
| 0001 Migration | Phase 1+2 (parallel) | 2 Parallel Subagents | 7 | ~2 min | ~8% |
| 0001 Migration | Phase 3+4+5 (parallel) | 3 Parallel Subagents | 12 | ~3 min | ~10% |
| Skill cleanup | 5 groups | 5 Parallel Subagents | 27 skills | ~3 min | ~12% |
